export function animateChildren(container: Element, duration: number = 300, easing: string = "ease-out") {
  interface ItemInfo {
    element: Element
  ...
  function getItemInfos(container: Element): ItemInfo[] {
    return Array.from(container.children).map((item) => {
      ...
      return {element: item, x: rect.left, y: rect.top, width: rect.width, height: rect.height}
    })
  }
