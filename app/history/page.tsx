...
async function Page() {
  ...
  return (
    <div className={clsx("w-[88%] max-w-2xl mx-auto py-6 sm:py-10")}>
      <table className={clsx("shadow w-full table-fixed")}>
        <thead>
        ...
        {data.map((value, index) => {
          ...
          return (<tr key={index} className="odd:bg-stone-50 even:bg-stone-100">
            <td className={clsx(tdStyle, "text-center")}>
              {sp.length == 2 && <>
                {sp[0]}
                <span className="text-sm block">{sp[1]}</span>
              </>}
            </td>
            <td className={tdStyle}>{value.prompt}</td>
            <td className={clsx(tdStyle, "text-center")}>
              {value.gua}
              <span className="text-xs block">{value.change}</span>
            </td>
          </tr>)
        })}
        </tbody>
